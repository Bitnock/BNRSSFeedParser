...
- (void)parserDidEndDocument:(NSXMLParser*)parser {
  ...
  if (_parsedElementStack.count > 0) {
    parseDict = _parsedElementStack[0];
    rssFeed = [[BNPodcastFeed alloc] initWithObjects:parseDict.allValues forKeys:parseDict.allKeys];
  }
  
  if (self.successBlock) {
    self.successBlock(self.operationResponse, rssFeed);
  }
}
